...
PATTERNS = {
    ...
    'evaluation': dict(label='Rubric-based evaluation', terms='rank rerank score rubric quality relevance evaluation benchmark assess priority', search='rubric evaluation ranking',
                   ...
                   caution='A high model probability is not proof of correctness. Test each dimension against representative human labels.',
...
NEIGHBORHOODS = {
    ...
    'media': ('mac-voice sde-vision-cascade computer-use-aaron', 'sde-vision-cascade'),
    'control': ('drone-sim robot-arm ariel-drone eve-commander physical-robot-arm', 'eve-commander drone-sim maze-failure npc-network-latency'),
    'memory': ('kamchatka-compaction hermes-compaction jevctl readybase', 'readybase'),
...
def answer(idea, use_jev=False, cases=None):
    ...
    return {'idea': idea, 'fit': fit, 'pattern': key, 'pattern_label': pattern['label'] if pattern else 'Clarification needed',
            ...
            'coverage': {'capture_cutoff': '2026-09-19T04:38:31.078Z', 'exhaustive': False,
                         'curated_cases': len(cases), 'unreviewed_urls': metrics.get('external_review_status_counts', {}).get('not_reviewed'),
